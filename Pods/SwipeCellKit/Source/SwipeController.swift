...
class SwipeController: NSObject {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        case .began:
            if let swipeable = scrollView?.swipeables.first(where: { $0.state == .dragging }) as? UIView, swipeable != self.swipeable as UIView? {
                return
            }
        ...
        case .changed:
            ...
            if swipeable.state == .animatingToCenter {
                ...
                if let swipedCell = swipedCell, swipedCell != self.swipeable as UIView? {
                    return
                }
            }
